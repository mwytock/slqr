function [K,h] = lqrsp_newton(A,B,W,Q,R,Lambda,K0,params)
...
for t=1:params.max_iters
  ...
  [D1,D2] = newton_cd(U,S,V,B,W,Q,R,Lambda,K,L,P,G,params);

  [Ktemp1,Jtemp1] = lqrsp_newton_ls(At,B,W,Q,R,Lambda,J,G,K,D1,params);
  [Ktemp2,Jtemp2] = lqrsp_newton_ls(At,B,W,Q,R,Lambda,J,G,K,D2,params);
  gradient = Jtemp2 < Jtemp1;
  if gradient
    K = Ktemp2;
    fprintf('Using gradient\n');
  else
    K = Ktemp1;
  end
  h.gradient(t) = gradient;
end
